function generateSummary() {
    ...
    if (!contentTextarea || !summaryTextarea || !generateSummaryButton || !contentRegio) {
        return;
    }

    const content = contentTextarea.value.replace(/<[^>]*>|\\[[^\\]]*]/g, '');
	console.debug('Used content:', content); // debug
    const selectedRegios = Array.from(contentRegio.querySelectorAll('input[type="checkbox"]:checked')).map(checkbox => {
    ...
    jQuery.post(ajaxurl, {
        ...
        function typeCharacter() {
            if (i < summary.length) {
                ...
                if (summary.charAt(i - 1) === ' ') { // If the last character was a space, add extra delay
