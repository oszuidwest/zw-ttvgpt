...
class TTVGPTApiHandler {
    ...
    public function generate_summary(string $content, int $word_limit): array {
        ...
        $this->logger->debug('Starting API request', array(
            'model' => $this->model,
        ...
        if ($status_code !== 200) {
            ...
            $this->logger->error('OpenAI API error: HTTP ' . $status_code, array(
        ...
        if (!isset($data['choices'][0]['message']['content'])) {
            $this->logger->error('Unexpected API response structure', array(
                'response' => $data
            ));
        ...
        $this->logger->debug('Summary generated successfully', array(
